...
import type { Property, User as Profile, Inquiry } from '@/lib/types';
...
export default function AdminDashboard() {
  ...
  const loadAdminData = async () => {
    setLoading(true);
    try {
      const [statsRes, usersRes, propsRes, inqsRes]: any[] = await Promise.all([
        api.get('/admin/stats'),
        api.get('/admin/users'),
  ...
  const approveProperty = async (id: string) => {
    try {
      await api.put(`/admin/properties/${id}`, { status: 'active' });
      setProperties(properties.map((p) => p._id === id ? { ...p, status: 'active' } : p));
  ...
  const rejectProperty = async (id: string) => {
    try {
      await api.put(`/admin/properties/${id}`, { status: 'rejected' });
  ...
  const verifySeller = async (userId: string) => {
    try {
      await api.put(`/admin/users/${userId}`, { verificationStatus: 'verified' });
      setUsers(users.map((u) => u._id === userId ? { ...u, verificationStatus: 'verified' } : u));
  ...
  const suspendUser = async (userId: string) => {
    try {
      await api.put(`/admin/users/${userId}`, { verificationStatus: 'suspended' });
      setUsers(users.map((u) => u._id === userId ? { ...u, verificationStatus: 'suspended' } : u));
  ...
  if (user?.role !== 'admin' || user?.email !== 'vconectproperties@gmail.com') {
    return (
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center border border-red-100">
            <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6">
              <Shield className="h-10 w-10 text-[#D32F2F]" />
            </div>
          ...
          <div className="flex gap-1 bg-white rounded-lg p-1 border border-gray-100 mb-6 overflow-x-auto">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${activeTab === tab.id
                  ...
                  {users.slice(0, 5).map((u) => (
                    <div key={u._id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className={`text-xs ${getStatusColor(u.verificationStatus)}`}>{u.verificationStatus}</Badge>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {properties.filter((p) => p.status === 'pending_approval').slice(0, 5).map((p) => (
                    <div key={p._id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                      ...
                      <div className="flex gap-1">
                        <Button size="sm" className="h-7 bg-green-600 hover:bg-green-700" onClick={() => approveProperty(p._id)}>
                          <CheckCircle className="h-3.5 w-3.5" />
                        </Button>
                        <Button size="sm" variant="outline" className="h-7 text-red-500" onClick={() => rejectProperty(p._id)}>
                  ...
                  <tbody className="divide-y">
                    {users.map((u) => (
                      <tr key={u._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <Badge className={getStatusColor(u.verificationStatus)}>{u.verificationStatus}</Badge>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            {u.role === 'seller' && u.verificationStatus !== 'verified' && (
                              <Button size="sm" className="h-7 bg-green-600 hover:bg-green-700" onClick={() => verifySeller(u._id)}>
                                <CheckCircle className="h-3.5 w-3.5" />
                              </Button>
                            )}
                            <Button size="sm" variant="outline" className="h-7 text-red-500" onClick={() => suspendUser(u._id)}>
                              <XCircle className="h-3.5 w-3.5" />
                            </Button>
                          </div>
                        </td>
                  ...
                  <tbody className="divide-y">
                    {properties.map((p) => (
                      <tr key={p._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-gray-500">
                          {(p.sellerId as any)?.fullName || 'Unknown'}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                  ...
                  <tbody className="divide-y">
                    {inquiries.map((inq) => (
                      <tr key={inq._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">{(inq.propertyId as any)?.title || 'N/A'}</td>
