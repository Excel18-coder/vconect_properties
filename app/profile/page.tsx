...
export default function ProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else { toast.success('Profile updated successfully'); await refreshProfile(); }
    setLoading(false);
  };
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div><Label className="flex items-center gap-2"><User className="h-4 w-4 text-gray-400" />Full Name</Label><Input value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} className="mt-1.5" /></div>
                ...
                <div><Label className="flex items-center gap-2"><Building2 className="h-4 w-4 text-gray-400" />Agency Name</Label><Input value={form.agency_name} onChange={(e) => setForm({ ...form, agency_name: e.target.value })} className="mt-1.5" placeholder="If applicable" /></div>
              ...
              <div><Label>Business Details</Label><textarea value={form.business_details} onChange={(e) => setForm({ ...form, business_details: e.target.value })} className="w-full mt-1.5 px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D32F2F] text-sm min-h-[100px]" placeholder="Tell us about your business..." /></div>
